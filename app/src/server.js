...
const api_key_secret = process.env.API_KEY_SECRET || 'callapp secret key';
...
io.sockets.on('connect', async (socket) => {
    log.debug('[' + socket.id + '] bağlantı kabul edildi', { host: socket.handshake.headers.host.split(':')[0] });

    socket.channels = {};
    sockets[socket.id] = socket;

    const transport = socket.conn.transport.name; // in most cases, "polling"
    log.debug('[' + socket.id + '] bağlantı transferi', transport);
    ...
    socket.conn.on('upgrade', () => {
        ...
        log.debug('[' + socket.id + '] bağlantı transferi yükseltildi', upgradedTransport);
    });
    ...
    socket.on('disconnect', async (reason) => {
        ...
        log.debug('[' + socket.id + '] bağlandı', { reason: reason });
    ...
    socket.on('join', async (config) => {
        ...
        if (channel in socket.channels) {
            log.debug('[' + socket.id + '] [Warning] zaten katıldınız', channel);
            return;
        }
        ...
        if (peers[channel]['lock'] === true && peers[channel]['password'] != channel_password) {
            log.debug('[' + socket.id + '] [Warning] Oda kitli', channel);
        ...
        log.debug('[Join] - odanın idsinden bağlanan peer', peers);
