...
function setButtonsToolTip() {
    ...
    // setTippy(audioBtn, 'right-start');
    // setTippy(videoBtn, 'Kameranı kapat', 'right-start');
    // setTippy(screenShareBtn, 'Ekran paylaş', 'right-start');
    // setTippy(recordStreamBtn, 'Kaydı başlat', 'right-start');
    // setTippy(fullScreenBtn, 'Tam ekran görüntüle', 'right-start');
    // setTippy(chatRoomBtn, 'Mesajlar', 'right-start');
    // setTippy(captionBtn, 'Open the caption', 'right-start');

    // setTippy(whiteboardBtn, 'Open the whiteboard', 'right-start');
    // setTippy(fileShareBtn, 'Dosya paylaş', 'right-start');
    // setTippy(mySettingsBtn, 'Open settings', 'right-start');

    // setTippy(leaveRoomBtn, 'Aramayı sonlandır', 'right-start');
    // chat room buttons

    // setTippy(msgerCPBtn, 'Gizli mesajlaşma', 'top');
    // setTippy(msgerClean, 'Mesajları temizle', 'top');
    // setTippy(msgerSaveBtn, 'Mesajları kaydet', 'top');
    // setTippy(msgerClose, 'Kapat', 'top');
    // setTippy(msgerEmojiBtn, 'Emojiler', 'top');
    // setTippy(msgerSendBtn, 'Gönder', 'top');
    // caption buttons
    // setTippy(captionTheme, 'Hayatlet tema', 'top');
    // setTippy(captionClean, 'Mesajları temizle', 'top');
    // setTippy(captionSaveBtn, 'Mesajları kaydet', 'top');
    // settings
...
function initClientPeer() {
    if (!isWebRTCSupported) {
        userLog('error', 'Bu tarayıcı Web RTC desteklemiyor, Lütfen başka bir tarayıcıdan deneyiniz!');
        return;
    }
    ...
    console.log('01. signaling server e bağlan');
    ...
    console.log('02. Bağlantıyı taşı', transport);
    ...
    signalingSocket.io.engine.on('upgrade', () => {
        ...
        console.log('Arttırılmış bağlantıyı taşıma', upgradedTransport);
    });
...
async function handleConnect() {
    console.log('03. Signaling server e bağlandı');

    myPeerId = signalingSocket.id;
    console.log('04. Peer id değerim [ ' + myPeerId + ' ]');
...
function handleServerInfo(config) {
    ...
    console.log('13. Peer sayısı', peers_count);
...
async function joinToChannel() {
    console.log('12. aramaya katıl', roomId);
    sendToServer('join', {
        channel: roomId,
        userAgent: userAgent,
...
//         color: 'black',
//         position: 'center',
//         title: '<strong style="color: black">Hoşgeldiniz</strong>',
//         imageAlt: 'temsilci bekleniyor...',
//         imageUrl: welcomeImg,
//         html:
//             `
//         <br/> 
//         <p style="color:black;">Temsilcimiz en kısa sürede aramaya katılacaktır Lütfen bu esnada ekranda bekleyiniz.</p>
//         <p style="color:rgb(8, 189, 89);">
//             </p>`,
//         showCancelButton: true,
//         cancelButtonText: `Beklemeye devam et`
//     }) 
// }
...
async function handleAddPeer(config) {
    ...
    if (peer_id in peerConnections) {
        ...
        console.log('Zaten bağlısınız', peer_id);
        return;
    }
    ...
    console.log('[RTCPeerConnection] - tarafına bağlandı', JSON.stringify(connectedPeersName));
    ...
    if (should_create_offer) {
        ...
        console.log('[RTCPeerConnection] - Bağlantı teklifi oluşturulamadı', {
            peer_id: peer_id,
            peer_name: peer_name,
        });
    }
...
async function handlePeersConnectionStatus(peer_id) {
    peerConnections[peer_id].onconnectionstatechange = function (event) {
        ...
        const peerName = "Temsilci";
        console.log('[RTCPeerConnection] - Bağlantı', {
            peer_id: peer_id,
            peer_name: peerName,
...
async function handleOnTrack(peer_id, peers) {
    ...
    peerConnections[peer_id].ontrack = (event) => {
        ...
        console.log('[ON TRACK] - bilgi', { peer_id: peer_id, peer_name: peer_name, kind: kind, track: event.track });
        if (event.streams && event.streams[0]) {
            ...
            console.log('[ON TRACK] - Ekran paylaşımı', { peer_id: peer_id, peer_name: peer_name, kind: kind });
...
async function handleAddTracks(peer_id) {
    ...
    await localMediaStream.getTracks().forEach((track) => {
        console.log('[ADD TRACK] Peer adı [' + peer_name + '] kind - ' + track.kind);
...
async function handleRtcOffer(peer_id) {
    ...
    peerConnections[peer_id].onnegotiationneeded = () => {
        console.log('RTC isteği oluşturuldu ' + allPeers[peer_id]['peer_name']);
        // https://developer.mozilla.org/en-US/docs/Web/API/RTCPeerConnection/createOffer
        peerConnections[peer_id]
            .createOffer()
            .then((local_description) => {
                console.log('Local istek açıklaması', local_description);
                ...
                peerConnections[peer_id]
                    ...
                    .then(() => {
                        ...
                        console.log('setLocalDescription isteği başarılı!');
                    })
                    .catch((err) => {
                        console.error('[Error] istekte hata setLocalDescription', err);
                        //    userLog('error', 'Offer setLocalDescription failed ' + err);
                    });
            })
            .catch((err) => {
                console.error('[Error] istek gönderilirken', err);
            });
    };
}
...
function handleSessionDescription(config) {
    console.log('Uzak Oturum Açıklaması', config);
    ...
    peerConnections[peer_id]
        ...
        .then(() => {
            console.log('setRemoteDescription başarılı!');
            if (remote_description.type == 'offer') {
                console.log('Cevap oluştur');
                // https://developer.mozilla.org/en-US/docs/Web/API/RTCPeerConnection/createAnswer
                peerConnections[peer_id]
                    .createAnswer()
                    .then((local_description) => {
                        console.log('cevap açıklaması: ', local_description);
                        // https://developer.mozilla.org/en-US/docs/Web/API/RTCPeerConnection/setLocalDescription
                        peerConnections[peer_id]
                            .setLocalDescription(local_description)
                            .then(() => {
                                ...
                                console.log('setLocalDescription cevabı oluşturması başarılı!');

                                // https://github.com/miroslavpejic85/mirotalk/issues/110
                                if (needToCreateOffer) {
                                    needToCreateOffer = false;
                                    handleRtcOffer(peer_id);
                                    console.log('[RTCSessionDescription] - TEKLİF OLUŞTURMA GEREKİYOR', {
                                        peer_id: peer_id,
                                    });
                                }
                            })
...
function handleDisconnect(reason) {
    console.log('signaling sunucusuyla bağlantı kesildi', { reason: reason });
...
function handleRemovePeer(config) {
    ...
    console.log('Tüm Peers', allPeers);
}
...
function setTheme(theme) {
    ...
    switch (theme) {
        case 'dark':
            // dark theme
            swalBackground = 'radial-gradient(#0F044C, #141E61)';
            document.documentElement.style.setProperty('--body-bg', 'radial-gradient(#0F044C, #141E61)');
            document.documentElement.style.setProperty('--msger-bg', 'radial-gradient(#EEEEEE, #EEEEEE)');
            document.documentElement.style.setProperty('--msger-private-bg', 'radial-gradient(#393939, #141E61)');
            document.documentElement.style.setProperty('--wb-bg', 'radial-gradient(#393939, #141E61)');
            document.documentElement.style.setProperty('--box-shadow', '0px 8px 16px 0px rgba(0, 0, 0, 0.2)');
            document.documentElement.style.setProperty('--left-msg-bg', '#393939'); // left message background color
            document.documentElement.style.setProperty('--right-msg-bg', '#141E61'); // right message background color
            document.documentElement.style.setProperty('--private-msg-bg', '#6b1226'); // private message background color
            document.body.style.background = 'radial-gradient(#393939, #141E61)';
            break;
        case 'grey':
            // grey theme
...
function setButtonsBarPosition(position) {
    ...
    mirotalkBtnsBar = 'horizontal'; // her zaman yatay gelsin
    switch (mirotalkBtnsBar) {
        ...
        case 'horizontal':
            ...
            document.documentElement.style.setProperty('--btns-width', '500px');
            document.documentElement.style.setProperty('--btns-flex-direction', 'row');
            break;
        default:
            console.log('Pozisyon hatası');
    }
}
...
async function initEnumerateDevices() {
    console.log('05. init Numaralandırma Cihazları');
...
function enumerateAudioDevices(stream) {
    console.log('06. Ses Cihazlarını çağır');
...
function enumerateVideoDevices(stream) {
    console.log('07. Video cihazlarını çağır');
    navigator.mediaDevices
        .enumerateDevices()
        .then((devices) =>
            devices.forEach((device) => {
...
async function setupLocalMedia() {
    ...
    console.log('08. Yerel ses - video girişlerine erişim talep etme');
    console.log('09. desteklenenler', navigator.mediaDevices.getSupportedConstraints());
...
async function loadLocalMedia(stream) {
    console.log('10. Mikrofon ve Kamera izinleri verildi');
    // hide loading div
    getId('loadingDiv').style.display = 'none';

    localMediaStream = stream;

    console.log('YEREL MEDYA AKIŞ PARÇALARINI YÜKLE', localMediaStream.getTracks());
...
async function loadRemoteMediaStream(stream, peers, peer_id) {
    ...
    console.log('UZAKTAN MEDYA AKIŞ PARÇALARINI YÜKLE - PeerName:[' + peer_name + ']', remoteMediaStream.getTracks());
...
function handleVideoPlayerFs(videoId, videoFullScreenBtnId, peer_id = null) {
    ...
    function showMsg() {
        userLog('toast', 'Video açıkken tam ekran modu çalıştırılabilir.');
    }
...
function handleFileDragAndDrop(elemId, peer_id, itsMe = false) {
    ...
    videoPeer.addEventListener('drop', function (e) {
        e.preventDefault();
        if (itsMe) {
            userLog('warning', 'Kendinize dosya gönderemezsiniz.');
            return;
        }
        if (sendInProgress) {
            userLog('warning', 'Lütfen önceki dosyanın gönderilmesini bekleyin.');
            return;
        }
        if (e.dataTransfer.items && e.dataTransfer.items.length > 1) {
            userLog('warning', 'Aynı anda sadece bir adet dosya gönderilebilir.');
            return;
        }
        ...
        if (e.dataTransfer.items) {
            ...
            if (item.isDirectory) {
                userLog('warning', 'Direkt klasör gönderilemez, Bir dosya seçin örneğin xx.png', 'top-end');
                return;
            }
...
function handleVideoToImg(videoStream, videoToImgBtn, peer_id = null) {
    ...
    videoBtn.addEventListener('click', () => {
        ...
        //  userLog('toast', 'Anlık görüntü aktarımı çalışmıyor.');
    });
}
...
function takeSnapshot(video) {
    ...
    // console.log(dataURL);
    saveDataToFile(dataURL, getDataTimeString() + '-Görüntüsü.png');
}
...
function setChatRoomBtn() {
    ...
    //        // userLog('info', 'Aramada başkası bulunmuyor');
    //         return;
    //     }
...
function getVideoConstraints(videoQuality) {
    ...
    // videoQuality değişkenı eklenırse el ile secılebılır ayarlar aktıf edıldıkten sonra
    //  videoQuality = 'fhdVideo';
    switch (videoQuality) {
        case 'default':
...
function gotDevices(deviceInfos) {
    ...
    for (let i = 0; i !== deviceInfos.length; ++i) {
        ...
        switch (deviceInfo.kind) {
            case 'videoinput':
                option.text = `📹 ` + deviceInfo.label || `📹 Kamera ${videoSelect.length + 1}`;
                videoSelect.appendChild(option);
                break;

            case 'audioinput':
                option.text = `🎤 ` + deviceInfo.label || `🎤 Mikrofon ${audioInputSelect.length + 1}`;
                audioInputSelect.appendChild(option);
                break;

            case 'audiooutput':
                option.text = `🔈 ` + deviceInfo.label || `🔈 Hoparlör ${audioOutputSelect.length + 1}`;
...
function copyRoomURL() {
    ...
    // userLog('toast', 'Arama linki kopyalandı 👍');
}
...
function handleVideo(e, init, force = null) {
    ...
    if (init) {
        ...
        if (!isMobileDevice) {
            setTippy(initVideoBtn, myVideoStatus ? 'Kamerayı kapat' : 'Kamerayı aç', 'top');
        }
    }
...
async function toggleScreenSharing() {
    ...
    try {
        ...
        console.error('[Error] Ekran paylaışımı şuan da kullanılamıyor', err);
...
function handleMediaRecorderStop(event) {
    ...
    // myVideoParagraph.innerHTML = myPeerName + ' (sizin adınız)';
    myVideoParagraph.innerHTML = ' Yedaş Temsilcisi ';
...
function handleDataChannelFileSharing(data) {
    ...
    receiveFilePercentage.innerHTML =
        'İlerleme durumu: ' + ((receivedSize / incomingFileInfo.fileSize) * 100).toFixed(2) + '%';
...
function sendFileData(peer_id, broadcast) {
    console.log('Dosya gönder ' + fileToSend.name + ' boyutu ' + bytesToSize(fileToSend.size) + ' tipi ' + fileToSend.type);
    ...
    fileReader.addEventListener('error', (err) => console.error('dosya okuma hatası.', err));
    fileReader.addEventListener('abort', (e) => console.log('dosya gönderme iptal edildi.', e));
    fileReader.addEventListener('load', (e) => {
        ...
        sendFilePercentage.innerHTML = 'Gönderiliyor: ' + ((offset / fileToSend.size) * 100).toFixed(2) + '%';
    ...
    if (isImageURL(incomingFileInfo.fileName)) {
        ...
        reader.onload = (e) => {
            Swal.fire({
                ...
                position: 'center',
                title: 'Alınan dosya',
                text: incomingFileInfo.fileName + ' boyutu ' + bytesToSize(incomingFileInfo.fileSize),
                showDenyButton: true,
...
function kickOut(peer_id) {
    ...
    Swal.fire({
        ...
        position: 'center',
        imageUrl: confirmImg,
        title: 'Aramasını kapat ' + pName,
        text: 'Bu katılımcıyı atmak istediğinizden emin misiniz?',
        showDenyButton: true,
        confirmButtonText: `Evet`,
        denyButtonText: `Hayır`,
        showClass: {
...
function handleKickedOut(config) {
    ...
    Swal.fire({
        ...
        html:
            ...
            `User ` +
            peer_name +
            `</h2> sonra senin bağlantını kesecek <b style="color: #FF2D00;"></b> milisaniye.`,
        timer: 10000,
